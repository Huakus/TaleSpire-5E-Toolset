...
# Compute SHA-256 of a byte array (for local change detection).
function Get-BytesChecksum($bytes) {
...
function Invoke-SyncTick {
    ...
    if (Test-Path $SyncDir) {
        $localBytes    = Get-DirectoryZipBytes
        $localChecksum = Get-BytesChecksum $localBytes
